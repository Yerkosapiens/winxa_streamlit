...
def draw_side_bar():    
    ...
    ## Cost Scheduler
    st.sidebar.header("💰 Tarea de costo programada")
    st.sidebar.text_input("✏️ Nombre de la tarea", key="cost_input")
    st.sidebar.button("➕ Agregar tarea", key="add_schedule_button", on_click=add_cost_schedule)

    ## Work Scheduler
    st.sidebar.header("📅 Tarea de trabajo programada")
    st.sidebar.text_input("✏️ Nombre de la tarea", key="schedule_input")
    st.sidebar.button("➕ Agregar tarea", key="add_work_schedule_button", on_click=add_work_schedule)

    ## File Saver
    st.sidebar.button("💾 Guardar archivo", key="save_file", on_click=save_file)
...
def execute():
    ...
    st.header(" ⚙️ Tareas programadas ")
    ...
    if session.isHealthDataLoaded:
        tab1, tab2, tab3 = st.tabs(["📊 Debug", "📈 Estadisticas", "📝 Tareas Programadas"])
        ...
        st.header("Paso 1: Cargue el modelo BIM en la página de inicio")
