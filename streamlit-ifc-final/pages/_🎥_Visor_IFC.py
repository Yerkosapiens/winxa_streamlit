...
def write_health_data():
    ...
    with row1_col2:
        st.button("Inspeccionar desde el ID", key="edit_object_button", on_click=get_object_data, args=(st.session_state.object_id,))
        data = get_psets_from_ifc_js()
        if data:
            st.button("Inspeccionar desde el Modelo", key="get_object_button", on_click=get_object_data, args=(data['id'],)) if data else ""
...
def execute():
    ...
    st.subheader("🎥 Visor de Modelo BIM IFC")
    if "ifc_file" in session and session["ifc_file"]:
        if "ifc_js_response" not in session:
            session["ifc_js_response"] = ""
        draw_3d_viewer()
        tab1, tab2 = st.tabs(["🧮 Propiedades", "🩺 Debugger"])
        with tab1:
            write_pset_data()
        with tab2:
            write_health_data()
    else:
        st.header("Paso 1: Cargue el Modelo BIM desde la página de Inicio ")
